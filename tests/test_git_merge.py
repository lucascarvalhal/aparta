"""Workspace gitconfigs: rendering, backups, and exact gitdir binding."""

import os
import subprocess
from pathlib import Path

from aparta.backends.git import reconcile_workspace_git, render_workspace_gitconfig
...
def test_render_workspace_gitconfig_contents():
    profile = make_profile(git_name="Ana")
    text = render_workspace_gitconfig(profile, Workspace("app", "/x/app", profile.name, ["git", "ssh"]))
...
def test_render_workspace_gitconfig_uses_profile_git_host():
    profile = make_profile(ssh_alias="gitlab.com-work", git_host="gitlab.com")
    text = render_workspace_gitconfig(profile, Workspace("app", "/x/app", profile.name, ["git", "ssh"]))
...
def test_reconcile_backs_up_the_existing_gitconfig(tmp_path: Path, monkeypatch):
    ...
    profile = make_profile(root=str(home / "pessoal"))
    reconcile_workspace_git({profile.name: profile}, {}, SafeWriter(), home=home)
