...
CATALOG: dict[str, dict[str, str]] = {
    "pt": {
        ...
        "[workspace] provider to enable.": "[workspace] provedor a habilitar.",
        "[red]Usage: aparta add [workspace] <provider>[/red]": "[red]Uso: aparta add [workspace] <provedor>[/red]",
        "[green]{provider}[/green] is already enabled in workspace '{workspace}'.": "[green]{provider}[/green] já está habilitado no workspace '{workspace}'.",
        "[green]{provider}[/green] enabled in workspace '{workspace}'.": "[green]{provider}[/green] habilitado no workspace '{workspace}'.",
        "Workspace or profile to reauthenticate (default: current workspace).": "Workspace ou perfil para reautenticar (padrão: workspace atual).",
        "Workspace or profile to inspect (default: current workspace).": "Workspace ou perfil para inspecionar (padrão: workspace atual).",
        "Print a compact prompt status.": "Mostra um status compacto para o prompt.",
        "Workspace: [bold]{name}[/bold]": "Workspace: [bold]{name}[/bold]",
        "Profile: [bold]{name}[/bold]": "Perfil: [bold]{name}[/bold]",
        "Providers: {providers}": "Provedores: {providers}",
        "Credential expires in [yellow]{remaining}[/yellow].": "A credencial expira em [yellow]{remaining}[/yellow].",
        "Credential is renewable automatically.": "A credencial é renovável automaticamente.",
        "Emit a complete shell transition, including stale-variable cleanup.": "Emite uma transição completa do shell, removendo também variáveis antigas.",
        "[red]Unsupported shell: {shell}[/red]": "[red]Shell não suportado: {shell}[/red]",
        "[green]Automatic zsh workspace activation installed.[/green]": "[green]Ativação automática de workspaces no zsh instalada.[/green]",
        "[green]Automatic zsh workspace activation is already installed.[/green]": "[green]A ativação automática de workspaces no zsh já está instalada.[/green]",
        "Enable a provider in the current or named workspace.": "Habilita um provedor no workspace atual ou informado.",
        "Reauthenticate the current workspace or an explicit target.": "Reautentica o workspace atual ou um destino explícito.",
        "Show workspace identity, providers and credential expiry.": "Mostra identidade, provedores e expiração das credenciais do workspace.",
        "Install automatic zsh activation when directories change.": "Instala ativação automática no zsh ao trocar de diretório.",
        "Only this provider (gcloud, gh, adc or aws).": "Apenas este provedor (gcloud, gh, adc ou aws).",
        "session expired; log in again": "sessão expirada; faça login de novo",
        "[green]aws:[/green] profile '{name}' is still valid": "[green]aws:[/green] o perfil '{name}' continua válido",
        "Opening the AWS SSO login for profile '{name}'...": "Abrindo o login SSO da AWS para o perfil '{name}'...",
        "[green]aws:[/green] profile '{name}' reauthenticated": "[green]aws:[/green] perfil '{name}' reautenticado",
        "[yellow]aws:[/yellow] profile '{name}' uses static keys; refresh them with `aws configure --profile {name}`": "[yellow]aws:[/yellow] o perfil '{name}' usa chaves estáticas; atualize com `aws configure --profile {name}`",
        "valid credential; any library without a profile env uses it": "credencial válida; qualquer biblioteca sem env de perfil usa ela",
        "expired credential": "credencial expirada",
        "credential of unknown health": "credencial de estado desconhecido",
        "parked by --secure; libraries fail loudly": "estacionado pelo --secure; bibliotecas falham na cara",
        "none; libraries without a profile env fail loudly": "não existe; bibliotecas sem env de perfil falham na cara",
        "[yellow]Risk:[/yellow] libraries (Terraform, Dataform, every Google SDK) outside a configured folder use the global ADC without asking. [bold]aparta fallback --secure[/bold] parks it, reversibly.": "[yellow]Risco:[/yellow] bibliotecas (Terraform, Dataform, qualquer SDK do Google) fora de uma pasta configurada usam o ADC global sem avisar. [bold]aparta fallback --secure[/bold] estaciona ele, de forma reversível.",
        "  - park the global ADC at {path} (--restore puts it back)": "  - estaciona o ADC global em {path} (o --restore devolve)",
        "[green]ADC parked:[/green] libraries outside a profile now fail loudly instead of borrowing it.": "[green]ADC estacionado:[/green] bibliotecas fora de um perfil agora falham na cara em vez de pegá-lo emprestado.",
        "[green]ADC restored:[/green] the global application credentials are back.": "[green]ADC restaurado:[/green] as credenciais de aplicação globais voltaram.",
        "Profile to use (default: the one owning the current folder).": "Perfil a usar (padrão: o dono da pasta atual).",
        "Profile to print (default: the one owning the current folder).": "Perfil a imprimir (padrão: o dono da pasta atual).",
        "Also export GITHUB_TOKEN from the profile's gh (opt-in: it exposes the token to child processes).": "Também exporta GITHUB_TOKEN a partir do gh do perfil (opcional de propósito: expõe o token aos processos filhos).",
        "Run a command with the profile's environment, exactly as the agents get it.": "Roda um comando com o ambiente do perfil, exatamente como os agentes recebem.",
        'Print export lines for scripts: eval "$(aparta env)".': 'Imprime as linhas de export para scripts: eval "$(aparta env)".',
        "[red]Nothing to run.[/red] Usage: aparta run -- <command> [args...]": "[red]Nada para rodar.[/red] Uso: aparta run -- <comando> [args...]",
        "[red]{provider} credential for workspace '{workspace}' requires login: {detail}.[/red] Run [bold]aparta login[/bold] in that workspace.": "[red]A credencial {provider} do workspace '{workspace}' exige login: {detail}.[/red] Rode [bold]aparta login[/bold] nesse workspace.",
        "[red]This folder belongs to no profile.[/red] Use --profile <name> or run from a configured folder.": "[red]Esta pasta não pertence a nenhum perfil.[/red] Use --profile <nome> ou rode de uma pasta configurada.",
        "Run a command with the folder's profile environment.": "Roda um comando com o ambiente do perfil da pasta.",
        "Print the profile's exports for scripts: eval \"$(aparta env)\".": "Imprime os exports do perfil para scripts: eval \"$(aparta env)\".",
        ...
        "[yellow]ADC:[/yellow] {detail}; renewing the application credentials...": "[yellow]ADC:[/yellow] {detail}; renovando as credenciais de aplicação...",
        "[green]ADC:[/green] application credentials derived from the gcloud login, no browser needed": "[green]ADC:[/green] credenciais de aplicação derivadas do login do gcloud, sem precisar do navegador",
        "reauth needed": "reautenticar",
        "unknown": "desconhecido",
        "[green]gcloud:[/green] application credentials renewed": "[green]gcloud:[/green] credenciais de aplicação renovadas",
        "[dim]In the browser, pick the account {account}.[/dim]": "[dim]No navegador, escolha a conta {account}.[/dim]",
